          ...
          <div className="px-10 py-8 max-h-[65vh] overflow-y-auto scrollbar-hide">
            {loading ? (
              ...
              <div className="space-y-10">
                ...
                {riadRooms.length > 0 && (
                  <div>
                    <p className="text-[10px] tracking-[0.3em] uppercase text-foreground/40 mb-5">Riad di Siena</p>
                    <div className="space-y-2">
                      {riadRooms.map((room) => (
                        <button
                          key={room.Room_ID}
                          onClick={() => openRoomBooking(room)}
                          className="w-full flex items-center justify-between py-4 px-5 bg-foreground/[0.02] hover:bg-foreground/[0.05] transition-all duration-200 group"
                        >
                          <div className="text-left">
                            <p className="font-serif text-foreground/90 group-hover:text-foreground transition-colors">{room.Name}</p>
                            <p className="text-[11px] text-foreground/40 mt-1">Main House · No. 37</p>
                          </div>
                          <div className="flex items-center gap-3">
                            <p className="text-sm text-foreground/50">from {formatPrice(parseFloat(room.Price_EUR))}</p>
                            <svg width="14" height="14" viewBox="0 0 14 14" fill="none" stroke="currentColor" strokeWidth="1.5" className="text-foreground/30 group-hover:text-foreground/60 transition-colors">
                ...
                {douariaRooms.length > 0 && (
                  <div>
                    <p className="text-[10px] tracking-[0.3em] uppercase text-foreground/40 mb-5">The Douaria</p>
                    <div className="space-y-2">
                      {douariaRooms.map((room) => (
                        ...
                        >
                          <div className="text-left">
                            <p className="font-serif text-foreground/90 group-hover:text-foreground transition-colors">{room.Name}</p>
                            <p className="text-[11px] text-foreground/40 mt-1">The Annex · No. 35</p>
                          </div>
                          <div className="flex items-center gap-3">
                            <p className="text-sm text-foreground/50">from {formatPrice(parseFloat(room.Price_EUR))}</p>
                            <svg width="14" height="14" viewBox="0 0 14 14" fill="none" stroke="currentColor" strokeWidth="1.5" className="text-foreground/30 group-hover:text-foreground/60 transition-colors">
                ...
                {kasbahExperience && (
                  <div>
                    <p className="text-[10px] tracking-[0.3em] uppercase text-foreground/40 mb-5">The Kasbah</p>
                    <button
                      onClick={openKasbahBooking}
                      className="w-full flex items-center justify-between py-4 px-5 bg-foreground/[0.02] hover:bg-foreground/[0.05] transition-all duration-200 group"
                    >
                      <div className="text-left">
                        <p className="font-serif text-foreground/90 group-hover:text-foreground transition-colors">{kasbahExperience.Name}</p>
                        <p className="text-[11px] text-foreground/40 mt-1">Draa Valley · {kasbahExperience.Duration} nights</p>
                      </div>
                      <div className="flex items-center gap-3">
                        <p className="text-sm text-foreground/50">from {formatPrice(parseFloat(kasbahExperience.Price_EUR))}</p>
                        <svg width="14" height="14" viewBox="0 0 14 14" fill="none" stroke="currentColor" strokeWidth="1.5" className="text-foreground/30 group-hover:text-foreground/60 transition-colors">
                          <polyline points="5,2 10,7 5,12" />
                        </svg>
                      </div>
                ...
                {desertTents.length > 0 && (
                  <div>
                    <p className="text-[10px] tracking-[0.3em] uppercase text-foreground/40 mb-5">The Desert Camp</p>
                    <div className="space-y-2">
                      {desertTents.map((tent) => (
                        ...
                        >
                          <div className="text-left">
                            <p className="font-serif text-foreground/90 group-hover:text-foreground transition-colors">{tent.Name}</p>
                            <p className="text-[11px] text-foreground/40 mt-1">Erg Chebbi · {tent.Level}</p>
                          </div>
                          <div className="flex items-center gap-3">
                            <p className="text-sm text-foreground/50">from {formatPrice(parseFloat(tent.Price_EUR))}</p>
                            <svg width="14" height="14" viewBox="0 0 14 14" fill="none" stroke="currentColor" strokeWidth="1.5" className="text-foreground/30 group-hover:text-foreground/60 transition-colors">
                              <polyline points="5,2 10,7 5,12" />
                            </svg>
                          </div>
